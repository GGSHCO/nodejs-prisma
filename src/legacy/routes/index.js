...
} = require("../syfForms/js/services_userAdd"); //  form Roles
...
const {
  services_userList_zohoData,
  services_userList_zohocontact,
  services_userList_getExistingPortalClients,
} = require("../syfReports/js/services_userList"); //
...
const {
  ...
  allContractsValidationChange,completeObservationWithRemarks,getProjectDataObservationNew,updateAllObservationObsNew,addObservationEntry
} = require("../dashboards/js/observationDashboard"); // PM Observation dashboard

/////////itReturnFillingSummary.js/////////
const {
fetchItReturns  
} = require("../dashboards/js/itReturnFillingSummary"); // PM Observation dashboard
//////////////end///////////////////////////
const {
  onloadTemplates,
  getTabCounts,
...
const {
  ...
  getUvFormData,
  updateFormData,
  saveSubform,getEmployeeTimesheetOptions
...
// projectdeck by sathish
const {
  getObservstionDetail,
  getMilestoneDetails,
  getChecklistDetails,
  ...
  getkycBlobUrl,
  getstoredBlobUrl,
  deleteBlobUrl,sanjay_getAssignedPortalUsers
} = require("../portal/js/projectdeck.js");
...
const {
  ...
  savepaymentPortal,
} = require("../portal/js/portalservice.js"); // portal service
...
app.post("/api/verifyPayment", async (req, res) => {
  ...
  if (expectedSignature === razorpay_signature) {
    try {
      ...
      console.log("Full payment details from Razorpay:", paymentDetails);
...
app.post("/api/uploadDocumentationfile", async (req, res) => {
  try {
    ...
    // By-Sathish : Remove spaces from the original file name(or else create issue while deleting in blob)
...
app.post("/api/deleteDocumentationfile", async (req, res) => {
  try {
    const { blobUrl } = req.body;
    console.log("Blob URL:", blobUrl);

    if (!blobUrl) {
      return res.status(400).send({ message: "Blob URL is required." });
    }

    const url = new URL(blobUrl);
    const pathParts = url.pathname.split("/");
    const blobName = pathParts.pop(); // Gets the last part of the path, which is the blob name
    const containerNameFromUrl = pathParts[1]; // Gets the container name from the URL
    if (containerNameFromUrl !== containerName) {
      return res.status(400).send({ message: "Invalid blob URL." });
    }
    const containerClient = blobServiceClient.getContainerClient(containerName);
    ...
    // Handle cases where the blob might not exist anymore
...
// blob delete function by sathish
// app.post("/api/deleteDocumentationfile", async (req, res) => {
//   try {
//     // Get blob name and team name from the request body
//     const { originalname, teamName } = req.body;
//     if (!originalname || !teamName) {
//       return res.status(400).send("Missing required fields: originalname or teamName.");
//     }

//     const containerClient = blobServiceClient.getContainerClient(containerName);
//     const blobName = `${originalname}`; // Or use your naming convention if needed
...
app.post("/api/pm_getAssignmentLidName", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await getAssignmentLidName(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});
...
app.post("/api/pm_saveProposal", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await saveProposal(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});
...
app.post("/api/pm_getClientProjects", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await getClientProjects(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/pm_getClientContract", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await getClientContract(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/getProjectsWithContactInfo", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await getProjectsWithContactInfo(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});
...
app.post("/api/getProjectUsers_ua", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await getProjectUsers_ua(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/recordUserInvitation", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await recordUserInvitation(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/productvs", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await productvs(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});
...
app.post("/api/productdata", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await productdata(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/saveServiceOrder", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await saveServiceOrder(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/assignProductsToClient", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await assignProductsToClient(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/getProductsForClientAssignment", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await getProductsForClientAssignment(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});
app.post("/api/addProductToClient", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await addProductToClient(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});
app.post("/api/removeProductFromClient", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await removeProductFromClient(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});
...
app.post("/api/reco_getSummary", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await checkSummary(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/reco_saveSummary", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await saveSummary(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});
...
app.post("/api/completeObservationWithRemarks", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await completeObservationWithRemarks(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

//////////////////////below code is added by Sanjay on july 11 2025 //////////////

app.post("/api/getProjectDataObservationNew", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await getProjectDataObservationNew(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/updateAllObservationObsNew", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await updateAllObservationObsNew(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });  
});
...
app.post("/api/addObservationEntry", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await addObservationEntry(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });  
});
/////////////////////end////////////////////
//////////////////fetchItReturns-summary created by sanjay on July 10 2025/////////



app.post("/api/fetchItReturns", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await fetchItReturns(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});
...
app.post("/api/pm_obsDashboard_getPettyTaskUpdates", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await pettyTaskStatus(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});
...
app.post("/api/getEmployeeTimesheetOptions", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await getEmployeeTimesheetOptions(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});
...
app.post("/api/records_getRecordsData", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await getRecordsData(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});
...
app.post(
  "/api/services_userList_getExistingPortalClients",
  async (req, res) => {
    let data = req.body.data;
    let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
    let response = await services_userList_getExistingPortalClients(params);
    let jsonData = JSON.stringify(response);
    let encryptResult = Buffer.from(jsonData).toString("base64");
    res.json({ data: encryptResult });
  }
...
app.post("/api/projectdesk_getChecklistDetails", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await getChecklistDetails(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});
...
app.post("/api/sanjay_getAssignedPortalUsers", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await sanjay_getAssignedPortalUsers(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/projectdesk_getTeamMembersDetails", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await getTeamMembersDetails(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/projectdesk_getTimeSheetDetails", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await getTimeSheetDetails(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/projectdesk_getDocumentDetails", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await getDocumentDetails(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});
app.post("/api/projectdesk_saveBlobUrl", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await saveBlobUrl(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/projectdesk_exportBlobUrl", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await exportBlobUrl(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/projectdesk_saveKycBlobUrl", async (req, res) => {
  let data = req.body.data;
  // console.log("Data passed: ",data)
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  // console.log("params: ",params)
  let response = await saveKycBlobUrl(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/projectdesk_getkycBlobUrl", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await getkycBlobUrl(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/projectdesk_getstoredBlobUrl", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await getstoredBlobUrl(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/projectdesk_deleteBlobUrl", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await deleteBlobUrl(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});
...
app.post("/api/speed_getGroupClientContact", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await getGroupClientContact();
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/speed_getAllContracts", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await getAllContracts(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/speed_getMatchingClientName", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await getClientMatchingName(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});
app.post("/api/speed_getProjects_clientname", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await getProjectsClientName(params);
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/speed_getPersonalDocuments", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let response = await getPersonalDocuments();
  let jsonData = JSON.stringify(response);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});
...
app.post("/api/portal_getServices", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await getServices(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});
app.post("/api/portal_getServiceCriteriaPayment", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await getServiceCriteriaPayment(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});
app.post("/api/portal_getZohoClientName", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await getZohoClientName(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/addServiceOrder", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await addServiceOrder(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/removeServiceOrder", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await removeServiceOrder(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/getServiceOrdersForClient", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await getServiceOrdersForClient(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});

app.post("/api/portal_paymentCreation", async (req, res) => {
  let data = req.body.data;
  let params = JSON.parse(Buffer.from(data, "base64").toString("utf-8"));
  let result = await savepaymentPortal(params);
  let jsonData = JSON.stringify(result);
  let encryptResult = Buffer.from(jsonData).toString("base64");
  res.json({ data: encryptResult });
});
