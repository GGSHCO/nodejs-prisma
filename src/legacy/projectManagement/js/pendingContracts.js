...
const {getEmployeeTimesheetOptions } = require("../../formTemplates/js/forms");
require("datejs");
...
async function deleteAllProjects({ rowId }) {
  try {
    ...
    let deleteContractAssignmentNature =
      Buffer.from(delAssign).toString("base64");
    let delContract = `Delete from AllContracts where urn='${rowId}'`;
    let deleteAllContracts = Buffer.from(delContract).toString("base64");
    let res = await queryGet([
      deleteMilestone,
      deleteContractAssignmentNature,
      deleteAllContracts,
    ]);
    return res;
  } catch (error) {
...
async function duplicateContract({
  ...
  try {
    let allContracts = await fetchTable(
      `select * from allcontracts where urn='${contractID}'`
    );
    let allContractAssignmentNature = await fetchTable(
      `select * from contractAssignmentNature where contractID='${contractID}'`
    );
    let allMilestoneSubform = await fetchTable(
      `select * from milestoneSubform where contractID='${contractID}'`
    );
    let contractId_max = await fetchTable(
      `SELECT MAX(urn) FROM AllContracts where userid='${userid}' and companyid='${companyid}'`
    );
    ...
    let con_query = `INSERT INTO AllContracts(urn,contractID,userid,companyname,companyid,clientName,entity,liveChurn,status,proposalStatus,remarks,termsConditions,addedUser,addedTime)
        values('${contractID_str}','${contractID_str}','${userid}','${companyname}','${companyid}','${
      allContracts[0].clientName
    }','${allContracts[0].entity}','${allContracts[0].liveChurn}','${
      allContracts[0].status
    }','${allContracts[0].proposalStatus}','${allContracts[0].remarks.replace(
      /'/g,
      "''"
    )}','${allContracts[0].termsConditions.replace(
      /'/g,
      "''"
    )}','${username}','${addedTime}')`;
    ...
    for (let contractAssign of allContractAssignmentNature) {
      ...
      let contract_Assign = `INSERT INTO ContractAssignmentNature(
            ...
            '${contractID_str}', '${contractAssign.sequence}', '${
        contractAssign.assignmentNature
      }', '${contractAssign.dateFormat}', '${
        contractAssign.feeEstimate
      }','${assignmentID_str}',
            '${contractAssign.type}', '${contractAssign.frequency}', '${
        contractAssign.amount
      }', '${contractAssign.relevantYear}', '${
        contractAssign.contractStartDate
      }',
            '${contractAssign.contractEndDate}', '${
        contractAssign.quarter
      }', '${contractAssign.managerResponsible}', '${
        contractAssign.personResponsible1
      }',
            '${contractAssign.personResponsible2}', '${
        contractAssign.personResponsible3
      }', '${contractAssign.entity}',
            '${contractAssign.clientName.replace(/'/g, "''")}', '${
        contractAssign.liveChurn
      }', '${contractAssign.remarks.replace(/'/g, "''")}', '${
        contractAssign.partner
      }', '${contractAssign.relevantStartDate}',
            '${contractAssign.relevantEndDate}', '${
        contractAssign.edoc
      }', '${companyname}', '${companyid}',
            '${username}', '${addedTime}'
        )`;
      let contractAssignmentQuery =
        Buffer.from(contract_Assign).toString("base64");
      ...
      for (let milestone of allMilestoneSubform) {
        if (contractAssign.assignmentID == milestone.assignmentID) {
          ...
          let milestoneSubform_query =
            Buffer.from(mile_query).toString("base64");
...
async function getContractData({ companyid }) {
  try {
    let getLeads_query = await fetchTable(
      `SELECT * FROM AllContracts WHERE status='Contract' and companyid='${companyid}' order by addedTime desc`
    );
...
async function contractCreation({ contractID }) {
  try {
    await exeQuery(
      `delete from allobservations where contractcode='${contractID}'`
    );
...
async function updateContract({ contractID }) {
  try {
    let res = await exeQuery(
      `update allContracts set status='ongoingContract' where urn='${contractID}'`
    );
    return res;
  } catch (error) {
    return { e: error };
...
async function getContractAssignmentNature(contractID) {
  try {
    let getAssignmentNature_res = await fetchTable(
      `SELECT * FROM ContractAssignmentNature WHERE contractID='${contractID}'`
    );
...
async function projectCreation(assignmentRow, milestoneRow) {
  ...
  if (
    ...
    relevantStartDate.toLocaleDateString()
  ) {
  ...
  let currentDate_dateFormat =
    crnt_date_yy + "-" + crnt_date_mm + "-" + crnt_date_dd;
  ...
  if (rows.type == "Recurring") {
    ...
    for (let k = 0; k < months; k++) {
      ...
      let current_projectEndDate = new Date(
        current_projectStartDate.add(frequency).month()
      );
      let lastDay = new Date(
        current_projectEndDate.getFullYear(),
        current_projectEndDate.getMonth() + 1,
        0
      );
      ...
      let projectStartMonth = current_projectStartDate_temp.toLocaleString(
        "en-GB",
        { month: "short" }
      );
      let projectStartYear = current_projectStartDate_temp
        .getFullYear()
        .toString()
        .slice(-2);
      ...
      let relevantStartDate =
        start_yyyy + "-" + (start_mm + 1) + "-" + start_dd;
      ...
      let durationMonthList = getProjectDuration(
        relevantStartDate,
        relevantEndDate
      );
      ...
      let proQuery = `INSERT INTO AllProjects(projectCode,assignmentID,contractID,assignmentNature,date,clientName,
            ...
            VALUES ('${projectCode_str}','${rows.assignmentID}','${
        rows.contractID
      }','${rows.assignmentNature.replace(
        /'/g,
        "''"
      )}','${projectDate}','${rows.clientName.replace(/'/g, "''")}',
            '${edoc}','${quarter}','${projectDescription.replace(
        /'/g,
        "''"
      )}','${rows.amount}','${rows.managerResponsible.replace(
        /'/g,
        "''"
      )}','${rows.personResponsible1.replace(/'/g, "''")}',
            '${rows.personResponsible2.replace(
              /'/g,
              "''"
            )}','${rows.personResponsible3.replace(/'/g, "''")}','${
        rows.contractStatus
      }','Pending','${durationObj}','${rows.companyName.replace(
        /'/g,
        "''"
      )}','Pipeline','Yet to complete','Live',
            '${rows.companyId}','${addedUser}','${currentDate}','${
        rows.partner
      }','${rows.remarks.replace(/'/g, "''")}','${rows.projectRemarks.replace(
        /'/g,
        "''"
      )}','${relevantStartDate}','${relevantEndDate}','${relevantYear}','${assignmentNatureDisplay}'
            )
            `;
      ...
      for (const element of milestone) {
        ...
        if (liRows.advance == "true" && i == 1) {
          let invQuery = `INSERT INTO AllInvoices(invoiceNumber,projectCode,assignmentID,contractID,clientName,
                        ...
                        VALUES ('${invNum_str}','${projectCode_str}','${
            rows.assignmentID
          }','${rows.contractID}','${rows.clientName.replace(/'/g, "''")}',
                        '${projectDate}','${edoc}','${Number(
            amnt
          )}','${rows.assignmentNature.replace(
            /'/g,
            "''"
            ...
            /'/g,
            "''"
          )}','Pending','${rows.companyName.replace(/'/g, "''")}',
                        '${
                          rows.companyId
                        }','${addedUser}','${currentDate}','${projectDescription.replace(
            /'/g,
            "''"
          )}','${rows.remarks.replace(/'/g, "''")}'
                        )
                        `;
        ...
        let observQuery = `INSERT INTO AllObservations(observationCode,projectCode,assignmentID,contractCode,
          ...
          rows.assignmentID
                    ...
                    '${rows.assignmentNature.replace(
                      /'/g,
                      "''"
                    )}','${projectDate}','${invoiceMile.replace(
          /'/g,
          "''"
        )}','No Value','${amnt}','No Value','No Value',
                   '${rows.clientName.replace(
                     /'/g,
                     "''"
                   )}','No Value','System','${rows.managerResponsible.replace(
          /'/g,
          "''"
        )}','${rows.personResponsible1.replace(/'/g, "''")}',
                    '${rows.personResponsible2.replace(
                      /'/g,
                      "''"
                    )}','${rows.personResponsible3.replace(
          /'/g,
          "''"
        )}', 'Milestone','Pending','${rows.companyName.replace(/'/g, "''")}',
                    '${
                      rows.companyId
                    }','${addedUser}','${currentDate}','${standardHours}','${projectDescription}',
                    '${milestoneNumber}','${edoc}','${rows.partner.replace(
          /'/g,
          "''"
        )}','${rows.remarks.replace(/'/g, "''")}'
                    )
                    `;
                    console.log(observQuery)
    ...
    let proQuery = `INSERT INTO AllProjects(projectCode,assignmentID,contractID,assignmentNature,date,clientName,
      ...
      rows.assignmentNature
    }','${projectDate_dateFormat}','${rows.clientName.replace(/'/g, "''")}',
        '${edoc_dateFormat}','${nonRecurring_quarter}','${projectDescription}','${
      rows.amount
    }','${rows.managerResponsible}','${rows.personResponsible1}',
        '${rows.personResponsible2}','${rows.personResponsible3}','${
      rows.contractStatus
    }','Pending','${durationObj}','${rows.companyName}',
        '${
          rows.companyId
        }','${addedUser}','${currentDate}','${rows.remarks.replace(
      /'/g,
      "''"
    )}','${rows.projectRemarks.replace(
      /'/g,
      "''"
    )}','Pipeline','Yet to complete','Live',
        '${rows.relevantStartDate}','${rows.relevantEndDate}','${
      rows.partner
    }','${assignmentNatureDisplay}'
        )`;
    let projectQuery = Buffer.from(proQuery).toString("base64");
    // let projectQuery = proQuery;
    projectInsertQueryList.push(projectQuery);
    ...
    for (const element of milestone) {
      ...
      if (liRows.advance == "true" && i == 1) {
        let invQuery = `INSERT INTO AllInvoices(invoiceNumber,projectCode,assignmentID,contractID,clientName,
          ...
          rows.assignmentID
                    ...
                    '${projectDate}','${edoc_dateFormat}','${Number(amnt)}','${
          rows.assignmentNature
        }','${invoiceMile}','Pending','${rows.companyName}',
                    '${
                      rows.companyId
                    }','${addedUser}','${currentDate}','${projectDescription}','${rows.remarks.replace(
          /'/g,
          "''"
        )}'
                    )
      ...
      let observQuery = `INSERT INTO AllObservations(observationCode,projectCode,assignmentID,contractCode,
        ...
        rows.assignmentID
            ...
            '${
              rows.assignmentNature
            }','${projectDate}','${invoiceMile}','No Value','${amnt}','No Value','No Value',
           '${rows.clientName.replace(/'/g, "''")}','No Value','System','${
        rows.managerResponsible
      }','${rows.personResponsible1}',
            '${rows.personResponsible2}','${
        rows.personResponsible3
      }', 'Milestone','Pending','${rows.companyName}',
            '${
              rows.companyId
            }','${addedUser}','${currentDate}','${standardHours}','${projectDescription}',
            '${milestoneNumber}','${edoc_dateFormat}','${
        rows.partner
      }','${rows.remarks.replace(/'/g, "''")}'
            )
            `;
  ...
  if (insertProjectCall.responseType === "SUCCESS") {
    ...
    if (insertObservation.responseType === "SUCCESS") {
      ...
      await exeQuery(
        `Delete from AllObservations where contractCode=${rows.contractID}`
      );
      await exeQuery(
        `Delete from AllProjects where contractID=${rows.contractID}`
      );
...
function getProjectDuration(projectDate, edoc) {
  ...
  let durationMonth =
    duration_ed.getMonth() -
    duration_sd.getMonth() +
    1 +
    12 * (duration_ed.getFullYear() - duration_sd.getFullYear());
...
async function updateBlobUrl({ pdfblobUrl, rowID }) {
  try {
    let insertProposalPdf = await exeQuery(
      `UPDATE AllContracts SET attachments='${pdfblobUrl}'  WHERE urn=${rowID}`
    );
...
const updateOngoing = async (params) => {
  try {
    ...
    for (let i of data) {
      ...
      let contractAssignment = Buffer.from(contractAssignmentQuery).toString(
        "base64"
      );
      ...
      if (i.status == "Pipeline") {
        res = await fetchTable(
          `select projectCode from allProjects where assignmentID='${i.assignmentID}' and projectStatus='Pipeline'`
        );
      } else if (i.status == "Pending") {
        res = await fetchTable(
          `select projectCode from allProjects where assignmentID='${i.assignmentID}' and status='Pending'`
        );
      }
      if (res.length > 0) {
        let projectIds = [];
        res.map((item) => {
          projectIds.push(`'${item.projectCode}'`);
        });
...
const timesheetAlert = async (params) => {
  try {
    ...
    let allemployees = await getEmployeeTimesheetOptions({
    ...
    allemployees.forEach((element) => {
      if (
        element.label &&
        !employeeList.includes(element.label) &&
        !recipientList.includes(element.value)
      ) {
